...
class MPPIController(Node):
    def __init__(self):
        ...
        self._device = 'cuda'
        self._goal_pos = torch.tensor([9.0, 10.0, 3.14], device=self._device, dtype=self._dtype)
        self._dt = 0.1  # time step
        self.current_state = torch.zeros(3, dtype=self._dtype, device=self._device)
    ...
    def setup_mppi(self, mode):
        nx = 3 #num state [x y theta]
        horizon = 15
        num_samples = 1000
        lambda_ = 1.0

        self.terminal_scale = 100.0
        self.distance_weight = 50.0
        self.angle_weight = 10.0

    
        if mode == "non_holonomic":
            self.u_min = torch.tensor([0.0, -2.0], device=self._device, dtype=self._dtype) # vx, w
            self.u_max = torch.tensor([2.0,  2.0], device=self._device, dtype=self._dtype) # vx, w
            noise_sigma = torch.diag(torch.tensor([0.5, 0.5], device=self._device, dtype=torch.double)) # vx, w
            ...
            self.u_min = torch.tensor([-2.0, -2.0, -2.0], device=self._device, dtype=self._dtype) # vx, vy, w
            self.u_max = torch.tensor([2.0, 2.0, 2.0], device=self._device, dtype=self._dtype) # vx, vy, w
            noise_sigma = torch.diag(torch.tensor([0.5, 0.5, 0.5], device=self._device, dtype=torch.double)) # vx, vy, w
        ...
        self.mppi_controller = mppi.KMPPI(
            dynamics=dynamics,
    ...
    def odom_callback(self, msg: Odometry):
        ...
        self.current_state = torch.tensor([pos.x, pos.y, yaw], dtype=self._dtype, device=self._device)
    ...
    def timer_callback(self):
        ...
        if self.mode == "non_holonomic":
            ...
            self.twist.angular.z = float(action[1])
        elif self.mode == "holonomic":
            self.twist.linear.x = float(action[0])
            self.twist.linear.y = float(action[1])
            self.twist.angular.z = float(action[2])
    ...
    def non_holonomic_dynamics(self, state: torch.Tensor, action: torch.Tensor, delta_t: float = None) -> torch.Tensor:
        """
        Differential drive dynamics (non-holonomic):
        - State:  [x, y, theta]
        - Action: [v, omega]
        ...
        v = torch.clamp(action[:, 0], self.u_min[0], self.u_max[0])
        omega = torch.clamp(action[:, 1], self.u_min[1], self.u_max[1])
        theta = self.angle_normalize(theta)
        
        new_x = x + v * torch.cos(theta) * dt
        new_y = y + v * torch.sin(theta) * dt
        new_theta = self.angle_normalize(theta + omega * dt)
    ...
    def holonomic_dynamics(self, state: torch.Tensor, action: torch.Tensor, delta_t=None) -> torch.Tensor:
        """
        Mecanum or omnidirectional drive dynamics (holonomic):
        - State:  [x, y, theta]
        - Action: [vx, vy, omega]
        where vx and vy are linear velocities in x and y directions,
        and omega is angular velocity (optional in basic form)
        """

        dt = delta_t or self._dt
        x, y, theta = state[:, 0], state[:, 1], state[:, 2]
        vx = torch.clamp(action[:, 0], self.u_min[0], self.u_max[0])
        vy = torch.clamp(action[:, 1], self.u_min[1], self.u_max[1])
        omega = torch.clamp(action[:, 2], self.u_min[2], self.u_max[2])
        theta = self.angle_normalize(theta)

        new_x = x + (vx * torch.cos(theta) - vy * torch.sin(theta))* dt
        new_y = y + (vx * torch.sin(theta) + vy * torch.cos(theta))* dt
        new_theta = self.angle_normalize(theta + omega * dt)  # Could add rotation dynamics if needed

        return torch.stack([new_x, new_y, new_theta], dim=1)

    def cost_function(self, state: torch.Tensor, action: torch.Tensor) -> torch.Tensor:
        """Goal tracking cost"""
    ...
    def terminal_cost(self, states, actions):
        return self.terminal_scale * torch.norm(states[..., -1, :2] - self._goal_pos[:2], dim=-1)
