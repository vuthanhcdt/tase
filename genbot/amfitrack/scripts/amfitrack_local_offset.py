...
class Gen3Publisher(Node):
    def __init__(self):
        ...
        self.human_distance = self.declare_parameter('human_distance', 0.6).get_parameter_value().double_value
    ...
    def timer_callback(self):
        ...
        if isinstance(packet.payload, amfitrack.payload.EmfImuFrameIdPayload):
            ...
            if self.left_hand:
                ...
                x_right = x_left
                y_right = y_left - self.human_distance
                ...
                x_left = x_right
                y_left = y_right + self.human_distance

                self.update_transforms(x=x_right, y=y_right, quat_z = quat_z, quat_w = quat_w, frame="human_right")
                self.update_transforms(x=x_left, y=y_left, quat_z = quat_z, quat_w = quat_w, frame="human_left")
            x_center = (x_right + x_left)/2.0
            y_center = (y_right + y_left)/2.0
            self.update_transforms(x=x_center, y=y_center, quat_z = quat_z, quat_w = quat_w, frame="human_link")
            self.publish_relative_position(x=x_center, y=y_center, quat_z = quat_z, quat_w = quat_w)
                    
        else:
            self.get_logger().info("Please restart the node and turn on the tag first")

    def update_transforms(self, x, y, quat_z, quat_w, frame):
        ...
        self.transformStamped.transform.translation.x = x
        self.transformStamped.transform.translation.y = y
        self.transformStamped.transform.translation.z = 0.0
        self.transformStamped.transform.rotation.z = quat_z
        self.transformStamped.transform.rotation.w = quat_w
        self.broadcaster.sendTransform(self.transformStamped)

    def publish_relative_position(self, x, y, quat_z, quat_w):
        """Calculate and publish the robot-to-human relative position."""
        x_human = x
        y_human = y
        theta_human = self.quaternion_to_yaw([0.0, 0.0, quat_z, quat_w])
