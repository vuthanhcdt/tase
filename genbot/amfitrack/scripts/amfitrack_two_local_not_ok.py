...
class Gen3Publisher(Node):
    def __init__(self):
        ...
        self.relativeState.header.frame_id=self.tf_frame_id
        self.relativeState.child_frame_id=self.tf_child_name
    ...
    def setup_gen3(self):
        ...
        # sensor_nodes = [node for node in nodes if node.name == "Amfitrack Sensor"]

        if len(nodes) == 3:
            self.device1 = amfitrack.Device(nodes[1])
            self.device2 = amfitrack.Device(nodes[2])
            conn.start()
            self.device1.calibrate()
            self.device2.calibrate()
            return True
        else:
    ...
    def timer_callback(self):
        ...
        if not self.device1 or not self.device1.packet_available():
            return
            
        if not self.device2 or not self.device2.packet_available():
            return
        distance1 = 0.0
        distance2 = 0.0
        packet1 = self.device1.get_packet()
        packet2 = self.device2.get_packet()
        # print(packet)
        if isinstance(packet1.payload, amfitrack.payload.EmfImuFrameIdPayload):
            payload1 = packet1.payload
            self.update_transforms(payload1,"human_link1")
            distance1 = math.sqrt(payload1.emf.pos_x**2+payload1.emf.pos_y**2)
            # self.publish_relative_position(payload1)
            # print(payload)
        else:
            self.get_logger().info("Please restart the node and turn on the tag first")
        

        if isinstance(packet2.payload, amfitrack.payload.EmfImuFrameIdPayload):
            payload2 = packet2.payload
            distance2 = math.sqrt(payload2.emf.pos_x**2+payload2.emf.pos_y**2)
            # print(payload)
            self.update_transforms(payload2,"human_link2")
            # self.publish_relative_position(payload2)
        else:
            self.get_logger().info("Please restart the node and turn on the tag first")

        if distance1<distance2:
            self.publish_relative_position(payload1)
            self.update_transforms(payload1,"human_link")
        else:
            self.publish_relative_position(payload2)
            self.update_transforms(payload2,"human_link")

    def update_transforms(self, payload, frame):
        """Update TF transformations."""
        self.transformStamped.header.stamp = self.get_clock().now().to_msg()
        self.transformStamped.header.frame_id = self.tf_frame_id
        self.transformStamped.child_frame_id = frame
